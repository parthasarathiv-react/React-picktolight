...
const SHELF_SECTION_WIDTH = '144px';
const SHELF_SECTION_HEIGHT = '52px';
...
function DrawerCell({ cupboardId, row, col, ledsPerDrawer, dense }) {
    const assignment = getDrawerAssignment(cupboardId, row, col);
    const [hovered, setHovered] = useState(false);

    const cellId = `${row}${String.fromCharCode(64 + col)}`;
    const ledSizeClass = dense ? "w-1 h-1" : "w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5";
    const labelClass = dense ? "text-[9px] top-0.5 left-1" : "text-[12px] sm:text-[15px] top-1 left-1.5";
    const ledStripClass = dense ? "mt-2 gap-px px-0.5" : "mt-4 gap-0.5 px-1";
    ...
    const colorMeta = getLedColor(assignment.ledColor);
    const activeCount = Math.min(assignment.activeLeds, ledsPerDrawer);
                    ...
                    return (
                        <div
                            ...
                            className={cn(
                                ...
                                ledSizeClass,
                                isActive
                                    ? cn(colorMeta.tailwind, "scale-110")
...
function CupboardBay({ cupboard, isActive, onSelect, bayRef }) {
    ...
    const assignmentsCount = Array.from({ length: computedShelves }).reduce((sum, _, rowIndex) => {
    ...
    return (
        ...
        >
            <div className={cn(
                "max-w-full bg-ot-surface-top border shadow-2xl flex flex-col overflow-hidden",
...
export default function Cupboard2D({ cupboards = [], controllerName, floorName, selectedCupboard, activeCupboardIdx = 0, onSelectCupboard }) {
    ...
    return (
        <div className="w-full h-full bg-ot-bg-mid  p-3 sm:p-4">
            <div className="h-full min-h-0 flex flex-col">
                ...
                <div
                    ref={scrollRef}
                    className="h-full min-h-0 flex p-2 flex-nowrap items-start gap-2 overflow-x-auto overflow-y-auto overscroll-contain cursor-grab active:cursor-grabbing select-none"
