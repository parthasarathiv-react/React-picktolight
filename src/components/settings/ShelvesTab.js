...
export default function ShelvesTab({ cupboardsData, syncCupboards, selectedCupboard, onSelectCupboard, wallsData, controllersData, refetchShelves }) {
    ...
    // Otherwise, show list of cupboards to choose from
    return (
        <div className="flex flex-col h-full space-y-6 animate-in fade-in p-6">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-xl font-semibold text-white">Shelves Designer</h3>
                    <p className="text-sm text-muted-foreground mt-1">
                        Select a cupboard to add and arrange shelves inside it.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredCupboards.map((cupboard) => {
                    const shelfCount = cupboard.shelfLayout ? cupboard.shelfLayout.length : (cupboard.shelves || 0);
                    return (
                        <button
                            key={cupboard.id}
                            onClick={() => onSelectCupboard(cupboard)}
                            className={cn(
